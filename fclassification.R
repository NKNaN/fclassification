...
for(b in 1:B){
  cat('b: ',b,'\n')
readpath <- paste0('G:/fclassification/sim_result_s',as.character(setting),'_d',as.character(delta),'/')
file_name_x <- paste0(readpath, 'sim_tau',as.character(tau),'_phi',as.character(phi),'_beta2_nongaussian_delta',as.character(delta),'_rn',as.character(rn),'_x_b', as.character(b),'.rds')
file_name_y <- paste0(readpath, 'sim_tau',as.character(tau),'_phi',as.character(phi),'_beta2_nongaussian_delta',as.character(delta),'_rn',as.character(rn),'_y_b', as.character(b),'.rds')
writepath <- paste0('G:/fclassification/sim_result_s',as.character(setting),'_d',as.character(delta),'_eva/')
file_name_evatxt <- paste0(writepath, 'evaluation_rn',as.character(rn),'.txt')
file_name_w_opt <- paste0(writepath, 'w_rn',as.character(rn),'_b',as.character(b),'.rds')
file_name_w_select <- paste0(writepath, 'w_select_rn',as.character(rn),'_b',as.character(b),'.rds')
X <- readRDS(file_name_x)
y <- readRDS(file_name_y)
  # y<-Y
plot(X[20,113,],type='l')

index <- sort(sample(nrow(X), nrow(X)/2))
X_train <- X[index,,]
y_train <- y[index]
X_test <-  X[-index,,]
y_test <- y[-index]
...
fit_try <- cv.glmnet(z, y,lambda = seq(0.01, 0.09, 0.02), nfolds = 5)
...
evaluate <- function(y_true, y_pred) {
  ...
  return(list('MCR' = mcr, 'Macro_P' = mp, 'Macro_R' = mr,
              'Macro_F1' = 2*mp*mr/(mp+mr) ))
}

# lasso_fit <- cv.projected_lasso(z, Y)
lasso_fit <- projected_lasso(z, Y, lambd = fit_try$lambda.min)
# w_opt <- lasso_fit$fit$w
w_opt <- lasso_fit$w_proj
# print(w_opt)
metrics <- evaluate(y_test, predict(z_test, w_opt))
print(metrics)

#outputs
write.table(metrics, file = file_name_evatxt,sep = '\t',
            append = T, col.names = F, row.names = F)
saveRDS(w_opt, file = file_name_w_opt)
saveRDS(lasso_fit$w_befporj, file = file_name_w_select)
}
end_time <- Sys.time()
